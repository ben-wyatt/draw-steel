    ...
    try:
        ...
        for model_info in models_data.get("data", []):
            ...
            if model_id == model:
                ...
                try:
                    input_price = float(pricing.get("prompt", 0.0))  # per million tokens
                    output_price = float(pricing.get("completion", 0.0))  # per million tokens
                ...
                if input_price > 0 or output_price > 0:
                    ...
                    _model_pricing_cache[model] = None
                    return None
        ...
        print(f"Unexpected error in get_model_pricing: {type(e).__name__}: {e}", file=sys.stderr)
        return None

...
def identify_table_cells(horizontal_lines: List[Dict], vertical_lines: List[Dict], 
                         page_rect: fitz.Rect, min_line_length: float = 20) -> List[Dict]:
...
def map_text_to_cells(text_blocks: List[Dict], cells: List[Dict]) -> List[Dict]:
    ...
    for block in text_blocks:
        ...
        for cell in cells:
            if (cell["x0"] <= b_center_x <= cell["x1"] and 
                cell["y0"] <= b_center_y <= cell["y1"]):
...
def analyze_page(doc: fitz.Document, page_num: int):
    ...
    if sig_h_lines:
        ...
        for i, line in enumerate(sig_h_lines[:10]):
            print(f"    {i+1}. y={line['y']:.2f}, x={line['x1']:.2f} to {line['x2']:.2f}, length={line['length']:.2f}")
    
    if sig_v_lines:
        print(f"\n  Significant vertical lines:")
        for i, line in enumerate(sig_v_lines[:10]):
            print(f"    {i+1}. x={line['x']:.2f}, y={line['y1']:.2f} to {line['y2']:.2f}, length={line['length']:.2f}")
    ...
    for i, block in enumerate(text_blocks):
        ...
        if block['lines']:
            ...
            if first_line['spans']:
                first_span = first_line['spans'][0]
                print(f"      First span font: {first_span['font']} @ {first_span['size']:.1f}pt")
        ...
        if ability['effect_block']:
            ...
            for j, line in enumerate(ability['effect_block']['lines']):
                if line['spans']:
                    ...
                    print(f"        Line {j+1} ({first_span['font']} @ {first_span['size']:.1f}pt): '{line['text']}'")
    ...
    if cells:
        ...
        for i, cell in enumerate(cells_with_content[:10]):
            ...
            print(f"      Position: ({cell['x0']:.1f}, {cell['y0']:.1f}) to ({cell['x1']:.1f}, {cell['y1']:.1f})")
    ...
    ability_keywords = ["Free Strike", "Melee", "Ranged", "Weapon", "Strike", "Action", "Trigger", "Effect"]
    relevant_words = [w for w in words if any(kw.lower() in w["text"].lower() for kw in ability_keywords)]

    ...
    for i, block in enumerate(text_blocks):
        ...
        if is_ability_title:
            ability_data = {
                ...
                "name": text,
                "title_block": block,
                "metadata_block": text_blocks[i + 1]
                if i + 1 < len(text_blocks)
                else None,
                "effect_block": text_blocks[i + 2]
...
def parse_metadata_block(metadata_block: Dict) -> Dict[str, Any]:
    ...
    for line in metadata_block["lines"]:
        ...
        elif "action" in text.lower() or text.lower() in [
            "maneuver",
            "reaction",
            "free maneuver",
        ]:
        ...
        # Range (starts with special char or "Melee" / "Ranged")
            ...
            text.startswith("e ") or "melee" in text.lower() or "ranged" in text.lower()
        ):
            metadata["range"] = text.lstrip("e ").strip()

        # Target (starts with special char or "One", "Each", "All", etc.)
        elif text.startswith("x ") or any(
            word in text.lower() for word in ["creature", "object", "ally", "enemy"]
        ):
        ...
        elif "cost" in text.lower() or any(
            word in text.lower() for word in ["hero token", "victory"]
        ):
            metadata["cost"] = text
...
def parse_effect_block(effect_block: Dict) -> Dict[str, Any]:
    ...
    effect = {"description": None, "power_roll": None, "tiers": {}, "raw_lines": []}
...
def extract_abilities_from_page(

"""
Approach 1: PyMuPDF with Column-Aware Sorting
Sort blocks by Y-coordinate first, then by X-coordinate within similar Y ranges.
Detect column boundaries using X-coordinate clustering.
Group paragraphs by section header within each column.
"""
...
def detect_columns(blocks: List[Dict], page_width: float) -> Tuple[float, float]:
    ...
    Returns (left_column_right_boundary, right_column_left_boundary)
    """
    x_positions = []
    for block in blocks:
        bbox = block["bbox"]
    ...
    # If gap is large enough (more than 20% of page width), it's likely columns
    if largest_gap[0] > page_width * 0.1:
        return (largest_gap[1], largest_gap[2])

    # Otherwise, assume single column centered
    return (page_width / 2, page_width / 2)


def get_column_for_block(
    block: Dict, left_boundary: float, right_boundary: float
) -> int:
...
def sort_blocks_column_aware(
    blocks: List[Dict], left_boundary: float, right_boundary: float
) -> List[Dict]:
    """
    Sort blocks: first by Y (top to bottom), then by X (left to right) within similar Y ranges.
    """
    Y_TOLERANCE = 10  # Points within this Y distance are considered same row
    ...
    def sort_key(block: Dict) -> Tuple[float, int, float]:
        ...
        # Round Y to nearest tolerance to group rows
...
def is_ability_block(block: Dict, block_index: int, all_blocks: List[Dict]) -> bool:
    ...
    is_ability_title = (
        "Newzald-Bold" in font_name and 9 <= font_size <= 11 and len(text) > 3
    )
...
def detect_section_header(block: Dict) -> Optional[str]:
    ...
    is_header = ("MCDM-Book" in font_name and 20 <= font_size <= 26) or (
...
def extract_rule_text_from_page(
    ...
    """Extract rule text with column-aware sorting."""
    page_rect = page.rect
    ...
    for i, block in enumerate(sorted_blocks):
        # Find original index for ability block check
        ...
        # Track column for section header association
        ...
        # Only associate content with subsection if in same column
...
def main():
    ...
    print(f"\n[V1: Column-Aware Sorting] Extracting from {HEROES_PDF}")
    ...
    try:
        ...
        output_file = output_dir / "extracted_rules_v1_column_sort.json"

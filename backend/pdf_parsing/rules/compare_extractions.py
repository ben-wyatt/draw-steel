...
def find_chunks_by_page(chunks: List[Dict[str, Any]], page_num: int) -> List[Dict[str, Any]]:
    """Find all chunks for a specific page."""
    return [chunk for chunk in chunks if chunk.get("page") == page_num]


def find_chunks_by_subsection(chunks: List[Dict[str, Any]], subsection: str) -> List[Dict[str, Any]]:
...
def analyze_page_266(chunks: List[Dict[str, Any]], version_name: str) -> Dict[str, Any]:
    ...
    for chunk in page_266_chunks:
        ...
        if subsection and "failure" in subsection.lower():
            ...
            if subsection and subsection.lower() not in ["failure", "failure with a consequence"]:
                break
            failure_chunks.append(chunk)
    ...
    for chunk in page_266_chunks:
        ...
        if "success with a reward" in section.lower() or "success with a reward" in subsection.lower():
...
def analyze_paragraph_merging(chunks: List[Dict[str, Any]], subsection: str) -> Dict[str, Any]:
    ...
    failure_text = " ".join(chunk.get("text", "") for chunk in subsection_chunks if "failure" in chunk.get("subsection", "").lower())
    ...
    keywords_found = [kw for kw in expected_keywords if kw.lower() in failure_text.lower()]
...
def generate_comparison_report() -> str:
    ...
    for version_num, version_name in versions:
        ...
        report_lines.append(f"\n{version_name} (V{version_num}):")
        report_lines.append(f"  Total chunks on page 266: {analysis['total_page_266_chunks']}")
        report_lines.append(f"  Failure chunks found: {analysis['failure_chunks_found']}")
        report_lines.append(f"  Failure section labeled correctly: {analysis['failure_section_labeled_correctly']}")
        
        # Show Failure chunks
        if analysis['failure_chunks']:
            report_lines.append("\n  Failure section chunks:")
            for i, chunk in enumerate(analysis['failure_chunks'][:5], 1):  # Show first 5
        ...
        if analysis['success_reward_chunks']:
            mislabeled = [chunk for chunk in analysis['success_reward_chunks'] 
    ...
    for version_num, version_name in versions:
        ...
        report_lines.append(f"\n{version_name} (V{version_num}):")
        report_lines.append(f"  Chunk count for 'Failure': {merging_analysis['chunk_count']}")
        report_lines.append(f"  Total text length: {merging_analysis['total_text_length']} chars")
        report_lines.append(f"  Expected keywords found: {merging_analysis['expected_keywords_found']}/{len(merging_analysis['expected_keywords'])}")
        ...
        missing = set(merging_analysis['expected_keywords']) - set(merging_analysis['keywords_found'])
        if missing:

...
from backend.agents.draw_steel_expert_class import DrawSteelExpert  # noqa: E402
...
def create_header_panel(collection_name: str, model_name: str) -> Panel:
    ...
    return Panel(
        ...
        title="[bold]🤖 Uglier Chat React[/bold]",
        padding=(1, 2),
    )
...
async def chat(collection_name: str, model_name: str):
    ...
    expert = DrawSteelExpert(collection_name=collection_name, model=model)
    current_session_id = expert.create_session()  # Create initial session
    console = Console()
    history = ChatHistory()
    ...
    console.print(
        ...
        "Type 'clear' to start a new session. Type 'help' for commands.[/dim]\n"
    )
    ...
    try:
        while True:
            try:
                ...
                # Handle clear command - create a new session
                if query.lower() == "clear":
                    current_session_id = expert.create_session()
                    history.clear()
                    console.print(
                        f"[dim]New session started: {current_session_id}[/dim]\n"
                    )
                    continue
                ...
                if query.lower().startswith("model "):
                    ...
                    if new_model_name in MODEL_MAP:
                        ...
                        model = MODEL_MAP[current_model_name]
                        # Close old expert and create new one with new model
                        expert.close()
                        expert = DrawSteelExpert(
                            collection_name=collection_name, model=model
                        )
                        current_session_id = (
                            expert.create_session()
                ...
                # Show thinking status - using DrawSteelExpert's run_agent with session
                with Status("[bold yellow]Thinking...", console=console):
                    result = await expert.run_agent(
                        query, session_id=current_session_id
                    )

                # Add response to history
                history.add("assistant", result.final_output)

                # Display response in a panel with Rich markdown
                console.print()
                console.print(
                    Panel(
